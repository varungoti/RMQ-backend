...
// Import the function directly since we're in the same directory
...
function runTests() {
  ...
  const result4 = createHybridResponse(testCase4, 'Custom props message', { customProp: 'test' });
  logResult(result4);
}
...
// Export to avoid TS error about the file being a module
